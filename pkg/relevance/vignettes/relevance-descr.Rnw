...
conclusion again. 
The independent repetition is called \T{replication} and provides new
data that can be similary to the data of the \T{original study} or quite
different. 

The section describes how to applied the relevance measure to the problem
...
Let $\IEff$ be the confidence interval in the replication and
$\IEDS$, the confidence interval for the difference.
...
ty <- structure(d.osc15Onesample[,c("effectr","teststatisticr","nr")],
      names=c("effect","teststatistic","n"))
( rr <- replication(tx, ty, rlv.threshold=0.1) )
plot(rr)
@ 
