...
\begin{itemize}
\item
a simple consistent way to specify plotting symbols, colors, sizes, and many more elements to tailor plots,
...
\item
flexible versions of plot matrices ("\T{pairs}") and conditional plot matrices ("\T{coplot}").
\end{itemize}
\end{abstract}
...
This basic function for plotting \T{y} on \T{x} is called \T{plyx}.
It includes plots of several \T{y}-variables---as does the standard
function \T{matplot}---and produces separate plots for multiple
\T{x}-variables (Section \ref{sec:multipleyx}).

When studying several variables, a popular standard function is \T{pairs}.
This package features \T{plmatrix}, extends the functionality of \T{pairs}.
Similarly, \T{plcond} corresponds to the standard function \T{coplot}
producing an array of scatterplots of \T{y} on \T{x} for the combination of
...
flaws are furnished by \T{plregr} which makes heavy use of the basic
function \T{plyx}.
...
may be made distinguishable by specific identifying symbols throughout 
a study. \T[Plgraphics] stores meta-data along with data.frames 
that avoids the need to repeatedly specify such graphical elements 
...
Clearly, this strongly resembles the result of simply calling \T{plot}, 
...
such that their area is proportional to its value.
The median size %is determined by \T{cex}. By default, this value 
...
\Tit{Groups.}
If the argument \T{by} is specified, separate plots will be generated
for the different groups specified by the levels of \T{by} variable, 
...
\Detail{The argument \T{mf} asks for multiple figures, saving space between panels. 
If left unspecified, \T{plyx} chooses a matrix of panels. 
Setting \T{mf=FALSE} avoids this.}
...
see more detail about the ``regular'' observations needs to generate a new
...
The plotting scales on the left and right side apply to the first two of 
...
The user can choose between plotting the terms or the variables
that are involved. 
...
Otherwise, the plotting variables are obtained by applying \T{all.vars} to
...
reflecting the original scale (as \T{log="y"} would do it for basic R).
\label{intro.plscale}
...
very rough image of the distribution.
...
\subsection{plmatrix}
The common type of scatterplot matrices are produced by the \T{pairs} function 
...
A second handsome function in basic graphics that produces a ``matrix''
...
The plgraphics version of \T{coplot} is called \T{plcond}.
...
and, in addition, some points of the neighboring intervals, with reduced size
%% (\T{ploptions("plcond.csize")}) 
and paled color. The color is changed to predefined colors indicating whether 
the point's conditioning variable(s) is (are) below or above the interval.
All these features are controlled by pl options.

In the example, the panel for species setosa and \T{Petal.Length} between
\T{2.18} and \T{3.36} only contains points from the neighboring interval
...
This can also be achieved in plain R by calling \T{termplot} for other fit objects,  
...
of the observation (by setting the argument \T{plregr(..., sequence=TRUE)}, 
...
points to a skewed distribution of the random deviations---that 
can also be seen in the qq plot. See the detailed description 
...
diagnostic tool to assess the validity of model assumptions.
It allows for detecting deviations from linearity, homoscedasticity,
symmetry and normality of the random deviations' distribution.
...
mimic the variability of this smooth line under the hypothesis that the
model is correct.
...
This display can also be requested from \T{plregr}
(by setting \T{plotselect=c(yfit=3)}).

In order to avoid the necessity to either call \T{plregr} again or to 
...
\Detail{
  ...
  They are obtained as follows: Residuals are calculated as the difference
  ...
  The two lines characterize how something like the low and high quartiles
  depend on the horizontal axis, and thus allow to examine the scale
  and skewness of the distribution of the random deviations.
}  
...
(QQ-plots for other distributions are lacking.)
Note that the residuals used here are NOT the raw or standardized residuals,
but instead, they are generated as follows:
The ``residuals from smooth'' as explained for the last plot are used to
estimate a ``smoothed scale'' by obtaining a smooth fit to 
the square roots of their absolute values against the model fit
and squaring the resulting smoothed values. 
They are then divided by these smooth-scale values in order to make them
homoscedastic even in the presence of a scale that depends on the model
fit. 
...
\]
are used, but weights are shown by the symbols' sizes.
...
\[
  D\sups C_i = \frac{r_i^2}{p\wh\sigma^2}\cdot\frac{h_i}{(1-h_i)^2}
    = {\textstyle \frac 1p} \; r_i^{*2} \frac{h_i}{1-h_i}
\;,
\]
...
plresx(r.blast)
@ 
The diagrams show a smooth and simulated smooth lines as described
...
Since this is an unusual law for residuals, it is convenient to transform
...
\Detail{
  ...
  uncensored ones: their \T{pcol} is paled by applying \T{colorpale}
...
Note that the smooth lines usually are increasing. 
As the simulated lines show, this does not means that there in any deficiency. 
The upward bump of the solid line, however, may be seen as a deviation from model assumptions.
...
\begin{itemize}
\item 
  The pl options are stored in a list \T{.ploptions} in the environment
  \T{pl.envir}.
\item
  There is a list \T{default.ploptions} in the package. It collects the
  package's default settings and is used as a backup if some components should
  not be contained in \T{.ploptions}.
\item
  Both of these lists can be overridden by objects with the same name
...
\item
  the \T{.ploptions} list in environment \T{pl.envir},
\item
  the list \T{default.ploptions} in the package \T{plgraphics}
...
\item
  \T{csize}, a factor applied to the current value of \T{par("cex")} to
  determine the general character expansion used by the pl functions.
\item
  \T{csize.pch}, the median character expansion.
  The default is the function \T{charSize}, with argument \T{n}, defined as
...
\item
  ...
  \T{pch, csize, csize.pch, csize.plab, lty, lwd, col}.\\
  \T{csize} is a factor which will be applied to \T{csize.pch} above for
  showing points by a single symbol (\T{pch}),\\
  \T{csize.plab} is an additional factor applied for the points that
  are shown by \T{plab}.
\item
  a group of components named \T{group} (\T{group.pch, ...}).
...
\item
  ...
  distinguishable;
\item
  \T{censored.pch} and \T{censored.csize} used to show 
...
\item
  \T{plext}: factor by which the range of the data should be
  extended unless an inner range is active (in which case the extension 
  is determined by \T{innerrange.ext}), and\\
  \T{plsymbolext}: further extension to allow for large symbols near the 
...
\item
  \T{title.csize} determines the character expansion of the plot title.
  By default, it adapts to the length of the title.
  For long titles, it will however never be smaller than
  \T{title.csizemin}. 
  If \T{title.csize} has 2 elements, the second refers to the subtitle.
\item
...
as the list \T{ploptions} in an environment called \T{pl.envir}.
They will be retrieved by subsequent low level pl functions,
like \T{plpoints}.
They can be inspected and changed by, e.g.,
\T{pl.envir\$mar} or \T{pl.envir\$mar\Gets c(5,3,2,1)}.
%% This may also help debugging when displays do not show what was desired.
(The list \T{.ploption}, with a leading dot, stores the options set by
the function \T{ploptions}.)
...
\Tit{Properties of observations.}
...
\item
  \T{psize}: plotting size, scaled by the pl option \T{csize};
\item
  \T{pcol}: color of the symbol. 
  This can be a factor. Then, the colors in \T{ploptions("group.color")}
  are used according to the \T{levels} of the factor.\\
  It can be numeric and should then contain low integer values.
  Values $>0$ are used again to select from \T{ploptions("group.color")};
  A value $0$ selects \T{ploptions("color")[1]} which is black unless changed.
  Negative values lead to a color of \T{NA} and will suppress plotting of
  the point.\\
  If \T{pcol} is logical, it is converted to \T{pcol+1} and selects the
...
@
%% table( current("pldata")[,".pch."] )  ## or 
As the last statement shows, the \T{pldata} dataset is available after
calling the plotting function in the environment \T{pl.envir}.
...
\item
  ...
  case these are not numeric or for other reasons, see \T{gendateaxis} below;
\item
  \T{plscale}: transformation function to generate the ``plotting scale'';    
\item
  \T{plvalues}: values used for plotting if different from \T{numvalues}, 
  typically transformed values;
\item
  \T{plcoord}: plotting coordinates, if different from \T{plvalues},
  typically when an inner plotting range is active.
\end{itemize}
Thus, plotting coordinates are defined as those first found searching for attributes \T{plcoord, plvalues, numvalues} and finally 
...
\item
  \T{vlim, vlimsc}: range within which the plotting should occur, that is,   \T{xlim} or \T{ylim}, if the variable is used as the horizontal and
...
\item
  ...
  positions of labels to indicate values of the variable, and the labels
  themselves;
\item
  \T{vpch, vlty, vcol}: plotting symbol, line type and color to be used 
  if multiple y's are shown in a plot.
  These elements are obtained by default from
  \T{ploptions("variables.xxx")}, where \T{xxx} is \T{pch}, \T{lty}, or
  \T{col}.
\end{itemize}
...
standardized form. 
It also takes any further arguments to be passed on to \T{ploptions}.
The result is stored in the \T{pl.envir} environment.
This allows for inspection of the plotting data
\T{pl.envir\$pldata} and the active \T{ploptions} (\T{pl.envir\$ploptions})
...
in the attribute \T{plvalues}, and setting 
...
according to their needs and they often set pl options. 
...
As an alternative to setting \T{keeppar=TRUE}, the function \T{plmarginpar}
...
These are available from \T{pl.envir\$ploptions} 
...
from those in \T{usr.ploptions}), then you need to set \T{marpar=FALSE}
in their call.
...
\T{plpoints} draws points and lines. \\
  ...
  is set (or \T{plargs\$pldata} contains a column named \T{".plab."}),
...
\item
  ...
  (given by the ploption \T{"title.csize"} is decreased
...
from \T{pl.envir\$pldata}, which is generated and stored by high level 
pl functions and will thus reflect the coordinates that are likely the
bases of the current plot. 
The pl options are also recovered from \T{pl.envir} (unless the argument
...
\item 
  \T{clipat}, drops data outside a range or replaces them with \T{NA}
  or a suitable value.
\item
...
\item
  ...
  no blueish color could be found that would be distinct enough from the 
  foregoing colors.
\end{itemize}
...
\item
  ...
  secondarily by the \T{vcol} attribute of the variables. 
  Thirdly, \Hneed{80mm} \T{ploptions("variables.col")} is used,
...
\T{vlim} and \T{innerrange} as attributes of variables using 
\T{setvarattributes} (or directly setting the attributes 
\T{attr(dd, "plrange")} or \T{attr(dd, "innerrange")}),
or by giving named lists of vectors of length 2
as arguments of high level pl functions.

The "variable limits" \T{vlim} can also be set as
\T{xlim} and \T{ylim} arguments of high level functions.
...
\T{ploptions("smooth.function")}, which defaults to \T{smoothRegr}, and that
...
version. This is reflected in \T{pl.envir\$pldata} and allows for
